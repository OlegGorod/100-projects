...
async function getMovieTrailer(movieId) {
  ...
  const regex = /trailer/i;
  const arrayOfTrailers = responseTrailer.results.filter(item => item.name.match(regex));
  const trailersKeys = arrayOfTrailers.filter(item => item.key);
  if (trailersKeys.length) {
    return trailersKeys[0].key;
  ...
  movie.results.forEach(element => {
    ...
    if (poster_path) {
      ...
      <img class="img_overview" id="${id}" src="${IMGPATH + poster_path}" alt="">
        ...
        `
      showModal(film, poster_path, title, vote_average, release_date, overview, id)
...
function showModal(film, poster_path, title, vote_average, release_date, overview, id) {
    ...
    <div class="overview modal">
      ...
      <div class="film_card">
        ...
        <div class="film_info">
          ...
          <a class="link film_trailer" href="https://www.youtube.com/embed/${await getMovieTrailer(id)}" target="_blank">${await formatBtnTrailer(id)}</a>
        </div>
      </div>
    </div>
    `
...
function addEllipsis(overview) {
  return overview.length > 430 ? overview.substring(0, 430) + `...` : overview;
}

async function formatBtnTrailer(id) {
  const check = await getMovieTrailer(id)
  if (check) {
    return 'Watch trailer'
  } else return 'No trailer'
}
